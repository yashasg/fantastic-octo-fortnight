...
final class TrueInterruptViewCoverageTests: XCTestCase {
    ...
    /// When the primary button is disabled (unavailable + no onSetUp), primaryButtonHintKey
    /// must return the disabled hint key (not the active-state preview hint key).
    ...
    /// When unavailable but onSetUp is provided (button enabled), active preview hint key is used.
    ...
    /// When approved, the enable hint key is used (button is enabled).
