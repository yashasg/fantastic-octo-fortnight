...
> **⚠️ Currently disabled in CI.** The `uitest-prepare`, `uitest-shard`, and `uitest`
> jobs in `.github/workflows/ci.yml` are gated with `if: false` while the
> MVVM → TCA migration ([#662](https://github.com/yashasg/fantastic-octo-fortnight/issues/662)) is in flight. The suite will be rewritten on
> top of TCA and re-enabled under [#736](https://github.com/yashasg/fantastic-octo-fortnight/issues/736). UI tests can still be built and
> executed locally via `./scripts/build.sh uitest`; they are simply not run on
> pull requests or `main` pushes.
...
✅ Sharded execution is supported for CI/local targeting, e.g.
