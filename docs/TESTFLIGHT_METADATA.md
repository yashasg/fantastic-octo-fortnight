...
kshana (क्षण, "a moment") is a lightweight iOS wellness app that helps you build healthier app-break habits. Set your eye and posture break timing, and kshana gently reminds you when it's time to step away. It pauses automatically when you're driving, on CarPlay, or in Focus Mode, so it only nudges when it matters. This beta uses local reminder alerts with Smart Pause integration. App-level app shielding is in development and will arrive in Phase 3 when Apple's entitlement approval is complete.
...
2. **Permission handling:** Test both "Allow Reminder Alerts" and "Not now" on Screen 2. Verify the app gracefully degrades to local backup alerts if notification access is not granted.
